...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.put(`http://${backendIpAddress}:${backendPort}/set_batch_status/${props.itemData.uuid}/${props.status}`, 
      '', {headers: authHeader()});
    toast.success(`Установлен статус ${props.status}`);      
    emit('docCreated'); // emit
    emit('closeModal')
  } catch (error) {
    console.error('Ошибка при установке статуса партии товаров', error);
...
<template>
  <div class="min-w-74 max-h-4/5 bg-white drop-shadow-md rounded-lg overflow-hidden">
    ...
    <header class="py-2 pl-6 bg-slate-200 text-black text-lg font-normal">
      Партия товаров #{{ props.itemData.id }}
