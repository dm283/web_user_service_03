...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.put(`http://${backendIpAddress}:${backendPort}/exit_prohibited/${props.itemData.id}`);
    toast.success('Выезд запрещён');      
...
<template>
  <div class="min-w-74 max-h-4/5 bg-white drop-shadow-md rounded-lg overflow-hidden">
    ...
    <div class="mt-5 mx-5">Запретить выезд ТС рег. № {{ itemData.ncar }} ?</div>
