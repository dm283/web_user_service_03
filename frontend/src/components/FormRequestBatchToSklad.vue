...
onMounted(async () => {
    try {
      ...
      form['carpass_uuid'] = response2.data.carpass_uuid
      state.initial_carpass_uuid = response2.data.carpass_uuid
      form['ncar'] = response2.data.ncar
      state.initial_ncar = response2.data.ncar
      }
...
const setInitialForm = () => {
  ...
  if (props.itemData) { // card and update
    for (let field of itemFields) {
      ...
      form['ncar'] = state.initial_ncar          // for dropdowns
    }
  } else {  // create
    for (let field of itemFields) {
      form[field] = null
      ...
      form['carpass_uuid'] = null          // for dropdowns
      form['ncar'] = null          // for dropdowns
    }
...
const handleSubmit = async () => {
  ...
  try {
    if (!props.isCard) {
      if (!props.itemData) {
        const response = await axios.post(`http://${backendIpAddress}:${backendPort}/requests_batch_to_sklad/`, 
    ...
    if (state.choosenDocs) {
      for (let doc of state.choosenDocs){
        ...
        formData2.append('obj_type_name', 'Заявки размещения партий на склад');
        formData2.append('obj_type', 'Заявки размещения партий на склад');
    ...
    emit('closeModal'); emit('openEditAfterCreate', state.responseItem, 'Заявки размещения партий на склад')
  } catch (error) {
...
<template>
  ...
  <div class="w-3/5 max-h-4/5 drop-shadow-md rounded-lg overflow-hidden" 
    ...
    <header class="py-2 pl-6 bg-slate-200 text-black text-lg font-normal">
      Заявка размещения партии на склад <span v-if="props.itemData">#{{ props.itemData.id }}</span>
    ...
    <form @submit.prevent="handleSubmit" enctype="multipart/form-data" class="mx-0 mt-5">
      ...
      <div class="flex">
        ...
        <div class="formInputDiv" v-if="(!props.isCard)">   <label class=formLabelStyle>Партия товаров (№ ТН, клиент)</label>
            <div :class=formInputStyle class="flex">
              ...
              <span class="ml-1 text-red-400 active:text-black" @click="showDropDownSelect['batch_input']=false; 
                  ...
                  form['carpass_uuid']=null;form['ncar']=null;">
          ...
          <div v-if="showDropDownSelect['batch_input']" class="bg-white border border-slate-400 rounded-md shadow-xl w-64 max-h-24 overflow-auto p-1 absolute z-10">
            <div class="px-1.5 py-0.5 cursor-pointer hover:bg-blue-300" v-for="item in state.filteredList" 
                @click="showDropDownSelect['batch_input']=false; 
                  ...
                  form['carpass_uuid']=item.carpass_uuid;form['ncar']=item.ncar;" >
        ...
        <div class=formInputDiv>   <label class=formLabelStyle>№ ТС</label>
          <input type="text" v-model="form.ncar" :class="[errField['carpass_uuid']==1 ? formInputStyleErr : formInputStyle]"
            :required="false" :disabled="true" />
        </div>
